...
<div class="layui-container">
    <div class="admin-login-background"><%--管理员登录后台--%>
        <div class="layui-form login-form"><%--layui-form 登录表单--%>
            <!-- action 属性定义在提交表单时执行的动作  method 属性规定在提交表单时所用的 HTTP 方法（GET 或 POST） -->
            <!-- 将表单数据数据提交到${pageContext.request.contextPath}/loginIn -->
            <form class="layui-form"  action="${pageContext.request.contextPath}/loginIn" method="post">
                <div class="layui-form-item logo-title"><%--layui-form-item 标志-标题--%>
                    <h1>图书管理系统</h1>
                    <div style="color: red;text-align: center;">${msg}</div>
                </div>
                <div class="layui-form-item"><!--用户名输入框-->
                    <label class="layui-icon layui-icon-username"></label><!--用户名图标-->
                    <input type="text" name="username" lay-verify="required" placeholder="用户名" autocomplete="off" class="layui-input" >
                </div>
                <div class="layui-form-item"><!--password输入框-->
                    <label class="layui-icon layui-icon-password"></label><!--password图标-->
...
<script type="text/javascript">
    ...
    function getCode(){
        /*
        控制层:LoginController-verifyCode获取验证码方法
         */
    ...
    function timestamp(url){
        var gettime=new Date().getTime();//getTime() 方法返回从 1970 年 1 月 1 日午夜到指定日期之间的毫秒数。
        if(url.indexOf("?")>-1){
